...
function LObject:runEvent2()
	...
	if rrr.magnitude > 0 then
		if self.root.direction.x == 1 then
			self.pic_offset_object.transform.rotation = CS.UnityEngine.Quaternion.Euler(0, 0, 360 - rrr.y)
		else
			self.pic_offset_object.transform.rotation = CS.UnityEngine.Quaternion.Euler(0, 180, rrr.y + 180)
		end
	else
		if self.root.direction.x == 1 then
			self.pic_offset_object.transform.rotation = CS.UnityEngine.Quaternion.Euler(0, 0, 0)
		else
			self.pic_offset_object.transform.rotation = CS.UnityEngine.Quaternion.Euler(0, 180, 0)
		end
	end
...
function LCharacterObject:new(parent, db, id, a, f, go, vx, vy, vz, k)
	local self = {}
	self = LObject:new(parent, db, id, a, f, go, vx, vy, vz, k)
	...
	self:addEvent("onBody", function(value)
		if self.bodyArray[value.id] == nil and not (value.x == 0 or value.y == 0 or value.width == 0 or value.height == 0) then
			self.bodyArray[value.id] = LColliderBDY:new(self.bdy_object, self.bdy_object_test)
	...
	self:addEvent("onTrigger", function(value)
		...
		self.physics_object.transform.position = CS.UnityEngine.Vector3(worldPos.x, utils.PLAYER.object.gameObject.transform.position.y, worldPos.y - utils.PLAYER.object.gameObject.transform.position.y)
...
function LCharacterObject:runFrame()
	...
	if self.kind == 5 or self.root ~= self then
		return
	end
	...
	for i, v in pairs(self.bodyArray) do
		local gg = v:BDYFixedUpdate(self.velocity)
		-- local gg, cc, ww, ee, eeaa, of = v:BDYFixedUpdate(self.velocity, self:getVar("weight"))

		-- self.rigidbody.position = self.rigidbody.position + of
		-- local pos2 = self.rigidbody.position
		-- self.bdy_object_test.transform.position = CS.UnityEngine.Vector3(pos2.x, pos2.y + pos2.z, 0)
