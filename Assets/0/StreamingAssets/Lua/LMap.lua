			...
			if gameMap[p][k] ~= 0 then -- 生成碰撞场景
				...
				if gameMap[p][k] ~= 4 then -- 梯子加碰撞，但不碰
					...
					if bit > 0 then
						...
						local boxCollider2D = unityobject_child:AddComponent(typeof(CS.UnityEngine.BoxCollider2D))
						boxCollider2D.offset = CS.UnityEngine.Vector2(boxCollider2D.offset.x, boxCollider2D.offset.y - ts.size / 100)
...
-- 0没有collider，0001=1上，0011=3上下，0111=7上下左，1111=上下左右，0101=5上下，1010=10左右
